...
**Architecture:** A small Spring Boot MVC app owns persistence through Spring Data JPA. A scheduled aggregation service fetches two exchanges and stores latest best bid/ask rows. A transactional trade service updates seeded wallet balances and records trade history.

**Tech Stack:** Java 17, Maven, Spring Boot 3.5.x, Spring Web, Spring Data JPA, H2, Bean Validation, JUnit 5, MockMvc.
...
- Create `src/main/java/com/aquariux/trading/config/DataSeeder.java`: seed demo user and wallet balances.
...
- Create `src/main/resources/application.properties`: H2 and app config.
...
- [ ] Add Maven Spring Boot project files and application entry point.
...
- [ ] Add `DataSeeder` to seed user `demo` with `50000` USDT and zero base assets.
...
- [ ] Add scheduler method with `@Scheduled(fixedRateString = "${app.price-refresh-ms:10000}")`.
